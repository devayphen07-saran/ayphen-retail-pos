...
export const createSupplierSchema = z.object({
  // Bounds mirror the suppliers PRD (§3/§21) — stricter than the backend base.
  name: z.string().trim().min(3, 'Name must be at least 3 characters').max(100),
  displayName: optionalOrEmpty(z.string().trim().max(50)),
  ...
  // The selected STATE lookup's guuid (→ state_lookup_guuid on the wire).
...
export const DEFAULT_CREATE_SUPPLIER_VALUES: CreateSupplierForm = {
  name: '',
  displayName: '',
  phone: '',
  email: '',
