...
import { ForbiddenError, PaymentRequiredError } from '#common/exceptions/app.exception.js';
...
export class StoreService {
  ...
  private async assertAccountCanWrite(accountId: string): Promise<void> {
    ...
    if (sub.status === 'expired') {
      throw new PaymentRequiredError(ErrorCodes.SUBSCRIPTION_PAYMENT_REQUIRED, 'Subscription payment required');
    }
    if (sub.accessValidUntil && sub.accessValidUntil < new Date()) {
      throw new PaymentRequiredError(ErrorCodes.SUBSCRIPTION_PAYMENT_REQUIRED, 'Subscription payment required');
    }
    if (sub.reconciliationStatus === 'pending') {
      throw new ForbiddenError(ErrorCodes.SUBSCRIPTION_RECONCILIATION_REQUIRED, 'Resolve your plan downgrade before creating stores');
    }
  }
  ...
  async createStore(userId: string, input: CreateStoreInput): Promise<CreatedStore> {
    ...
    if (!account) throw new ForbiddenError(ErrorCodes.NOT_ACCOUNT_OWNER, 'You are not the account owner');
    ...
    if (!this.entitlements.canCreate(precheckLimit, precheckActive)) {
      throw new ForbiddenError(ErrorCodes.STORE_LIMIT_REACHED, 'Store limit reached for this plan', {
        limit: precheckLimit,
